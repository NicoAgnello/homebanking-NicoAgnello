...
public class HomebankingApplication {
	...
	public CommandLineRunner initData(ClientRepository clientRepository,
		...
		return args -> {
			...
			Transaction transaction1 = new Transaction(LocalDateTime.now().plusMonths(2), -2000.12, TransactionType.DEBIT, "First transaction", account1.getBalance() - 2000.12);
			Transaction transaction2 = new Transaction(LocalDateTime.now().minusMonths(4), 3000, TransactionType.CREDIT, "Second transaction",2999.88 + 3000);
			Transaction transaction3 = new Transaction(LocalDateTime.now().plusHours(10), 1000.5, TransactionType.CREDIT, "Third transaction", account2.getBalance() + 1000.5);
			Transaction transaction4 = new Transaction(LocalDateTime.now().plusHours(2), -500, TransactionType.DEBIT, "Fourth transaction", account3.getBalance() - 500);
			...
			Card card2 = new Card(returnCvvNumber(),randomNumberCard(cardService), CardType.CREDIT, CardColor.TITANIUM,LocalDate.now(), LocalDate.now().plusYears(5).plusMonths(2), client1);
			Card card3 = new Card(returnCvvNumber(),randomNumberCard(cardService), CardType.CREDIT, CardColor.SILVER,LocalDate.now(), LocalDate.now().plusYears(5), client2);
			Card card4 = new Card(returnCvvNumber(),randomNumberCard(cardService), CardType.DEBIT, CardColor.TITANIUM,LocalDate.now(), LocalDate.now(), client1);
